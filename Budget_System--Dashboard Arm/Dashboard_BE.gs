...
const CONFIG = {
  ...
  DEMO_MODE: (function () {
    try {
      let isDemo =
        PropertiesService.getScriptProperties().getProperty("DEMO_MODE") ===
        "true";
      if (isDemo) {
  ...
  TAC_CATEGORY_WEIGHTS: {
    ...
    consumables: 0.2, // 20% for curriculum/supplies
  },
...
class KeswickDashboardService {
  constructor() {
    ...
    try {
      ...
      this.sheets = {
        ...
        organizationBudgets: this.getSheetSafely(
          "budget",
          "OrganizationBudgets",
        ),
  ...
  authenticateUser() {
    try {
      ...
      if (!userConfig) {
        if (!userInfo) {
          console.warn(
            `Unauthorized access attempt (Not in directory): ${email}`,
          );
        ...
        if (
          dirRole.includes("exec") ||
          dirRole.includes("cfo") ||
          dirRole.includes("head of school")
        ) {
          userConfig = {
            role: "executive",
            divisions: ["US", "LS", "KK", "AD"],
          ...
          dirRole.includes("princip") ||
          dirRole.includes("director")
        ) {
          userConfig = {
            role: "division",
            divisions: [rawDiv],
          ...
          console.warn(
            `Unauthorized access attempt (Role ${dirRole} not allowed): ${email}`,
          );
          return { email: email, authenticated: false, error: "ACCESS_DENIED" };
        }
      }
  ...
  calculateExecutiveKPIs(filters) {
    try {
      ...
      if (this.sheets.organizationBudgets) {
        ...
        this.cache.put(
          cacheKey,
          JSON.stringify(realKPIs),
          CONFIG.KPI_CACHE_DURATION,
        );
        return realKPIs;
      }
    ...
    try {
      ...
      return [
        ...
        },
        {
          ...
          description:
            utilization > 75 ? "Monitor closely" : "On track for fiscal year",
        },
        {
          ...
          description:
            pendingApprovals > 10 ? "Requires attention" : "Normal queue",
        },
      ];
    } catch (error) {
    ...
    try {
      ...
      for (let i = 1; i < data.length && transactions.length < limit; i++) {
        if (data[i][cols.TransactionID]) {
          const form = data[i][cols.Form] || "";

          transactions.push({
            transactionId: data[i][cols.TransactionID],
            orderId: data[i][cols.OrderID],
            date: data[i][cols.ProcessedOn], // standard UI mapped field
            processedOn: data[i][cols.ProcessedOn],
            requestor: data[i][cols.Requestor],
            approver: data[i][cols.Approver],
            division: "ALL", // placeholder
            department: data[i][cols.Organization] || "General", // map organization to department
            vendor:
              form === "AMAZON"
                ? "Amazon"
                : form === "WAREHOUSE"
            ...
            status: "Approved", // if in ledger, it is approved
    ...
    try {
      ...
      Object.values(divisionMap).forEach((division) => {
        if (division.allocated > 0) {
          division.utilization = Math.round(
            (division.spent / division.allocated) * 100,
          );
  ...
  mapOrganizationToDivision(organization) {
    ...
    if (orgLower.includes("keswick kids") || orgLower.includes("kk"))
      return "KK";
  ...
  getFinancialHealthMetrics(filters) {
    ...
    try {
      ...
      const expectedUtilization =
        CONFIG.BUDGET_PACING.milestones[fiscalMonth] ||
        (monthsElapsed / 12) * 100;
      ...
      return {
        ...
        metrics: {
          cashFlow:
            budgetRemaining > totalBudget * 0.3 ? "positive" : "constrained",
          burnRate:
            monthlyBurnRate > (totalBudget / 12) * 1.1 ? "elevated" : "normal",
    ...
    try {
      ...
      tacTransactions.forEach((row) => {
        ...
        if (
          ...
          org.includes("activit") ||
          desc.includes("field trip") ||
          desc.includes("activit")
        ) {
      ...
      const techAllocated = Math.round(
        totalCollected * CONFIG.TAC_CATEGORY_WEIGHTS.technology,
      );
      const activitiesAllocated = Math.round(
        totalCollected * CONFIG.TAC_CATEGORY_WEIGHTS.activities,
      );
      const curriculumAllocated = Math.round(
        totalCollected * CONFIG.TAC_CATEGORY_WEIGHTS.consumables,
      );
  ...
  getTACByGrade(filters) {
        ...
        data.forEach((row) => {
          ...
          grades.forEach((grade) => {
            const gradePattern = new RegExp(
              `\\b${grade}\\b|grade\\s*${grade}|gr\\.?\\s*${grade}`,
              "i",
            );
          ...
          if (matchedGrade && spendingByGrade[matchedGrade]) {
            ...
            if (
              form.includes("curriculum") ||
              desc.includes("textbook") ||
              desc.includes("workbook")
            ) {
              spendingByGrade[matchedGrade].curricular += amount;
            } else if (
              form.includes("field trip") ||
              desc.includes("field trip") ||
    ...
    const gradeData = grades.map((grade) => {
      ...
      const spending = spendingByGrade[grade] || {
        curricular: 0,
        fieldTrip: 0,
        techCost: 0,
        total: 0,
      ...
      const variancePercent =
        tacBudgeted > 0 ? (variance / tacBudgeted) * 100 : 0;
    ...
    const totals = {
      ...
      stepUpExpectedQuarterly: gradeData.reduce(
        (sum, g) => sum + g.stepUpExpectedQuarterly,
        0,
      ),
    };
  ...
  saveEnrollmentData(enrollmentByGrade) {
    try {
      ...
      return {
        success: false,
        message: "Failed to save enrollment data: " + error.message,
      };
    }
  }
      ...
      return orgData
        .map((org) => {
          ...
          const percentVariance =
            org.allocated > 0 ? (variance / org.allocated) * 100 : 0;
          let status = "on_track";
  ...
  getApprovalTurnaroundMetrics() {
    ...
    try {
      ...
      const submittedIdx =
        headers.indexOf("SubmittedOn") !== -1
          ? headers.indexOf("SubmittedOn")
          : headers.indexOf("Timestamp");
      const processedIdx =
        headers.indexOf("ProcessedOn") !== -1
          ? headers.indexOf("ProcessedOn")
          : -1;
      ...
      rows.forEach((row) => {
        ...
        const submittedOn = row[submittedIdx]
          ? new Date(row[submittedIdx])
          : null;
        const processedOn =
          processedIdx >= 0 && row[processedIdx]
            ? new Date(row[processedIdx])
            : null;
  ...
  getCostPerStudentMetrics() {
    const enrollment = this.getEnrollmentData();
    const totalEnrollment = Object.values(enrollment).reduce(
      (sum, count) => sum + count,
      0,
    );
    ...
    try {
      ...
      const totalSpent =
        budgetData.find((k) => k.id === "ytd_spending")?.value || 0;
      const overall =
        totalEnrollment > 0 ? Math.round(totalSpent / totalEnrollment) : 0;

      // Calculate by division
      const byDivision = Object.keys(CONFIG.DIVISIONS)
        .filter((d) => d !== "AD")
        .map((div) => {
          const divGrades = CONFIG.DIVISIONS[div].grades;
          const divEnrollment = divGrades.reduce(
            (sum, g) => sum + (enrollment[g] || 0),
            0,
          );
          // Estimate division spending proportionally (would need actual data for accuracy)
          const divSpent =
            totalEnrollment > 0
              ? Math.round(totalSpent * (divEnrollment / totalEnrollment))
              : 0;
          return {
            division: div,
            costPerStudent:
              divEnrollment > 0 ? Math.round(divSpent / divEnrollment) : 0,
  ...
  getStepUpPaymentStatus() {
    ...
    if (month >= 9)
      currentQuarter = 2; // Oct-Dec
    else if (month >= 0 && month < 3)
      currentQuarter = 3; // Jan-Mar
    ...
    return {
      ...
      quarters: {
        q1: {
          ...
          received: received.q1,
          variance: received.q1 - quarterlyExpected,
        },
        q2: {
          expected: quarterlyExpected,
          received: received.q2,
          variance: received.q2 - quarterlyExpected,
        },
        q3: {
          expected: quarterlyExpected,
          received: received.q3,
          variance: received.q3 - quarterlyExpected,
        },
        q4: {
          expected: quarterlyExpected,
          received: received.q4,
          variance: received.q4 - quarterlyExpected,
        },
      },
  ...
  getSpendingTrends(filters) {
    ...
    try {
      ...
      const orgData = this.getOrganizationBudgetData();
      const totalBudget = orgData.reduce(
        (sum, org) => sum + (org.allocated || 0),
        0,
      );
      ...
      const monthly = months
        ...
        .filter(
          (m) =>
            m.actual > 0 ||
            months.indexOf(m.month) <= this.getCurrentFiscalMonth(),
        );
      ...
      return {
        ...
        categorical:
          categorical.length > 0
            ? categorical
            : this.getMockCategoricalSpending(),
        lastUpdated: new Date().toISOString(),
      };
    } catch (error) {
  ...
  categorizeTransaction(org, description) {
    ...
    if (
      ...
      descLower.includes("computer") ||
      descLower.includes("software")
    ) {
      return "Technology";
    }
    if (
      orgLower.includes("curriculum") ||
      descLower.includes("textbook") ||
      descLower.includes("book")
    ) {
      return "Curriculum";
    }
    if (
      orgLower.includes("supply") ||
      orgLower.includes("supplies") ||
    ...
    if (
      orgLower.includes("activit") ||
      descLower.includes("field trip") ||
      descLower.includes("event")
    ) {
      return "Activities";
    }
    return "Other";
  ...
  getAdminExpensesReport(filters) {
    ...
    return {
      ...
      totalAmount: adminTransactions.reduce(
  ...
  getCurriculumReport(filters) {
    ...
    const curriculumTransactions = transactions.filter((t) => {
      ...
      return (
        form.includes("curriculum") ||
        org.includes("curriculum") ||
        org.includes("textbook") ||
        org.includes("workbook")
      );
    });
    ...
    return {
      ...
      totalAmount: curriculumTransactions.reduce(
        (sum, t) => sum + (t.amount || 0),
        0,
      ),
      byDepartment: Object.values(byDepartment).sort(
  ...
  getFieldTripReport(filters) {
    ...
    return {
      ...
      totalAmount: fieldTripTransactions.reduce(
        (sum, t) => sum + (t.amount || 0),
        0,
      ),
      byDivision: Object.values(byDivision).sort(
        (a, b) => b.totalAmount - a.totalAmount,
      ),
  ...
  getSupplyReport(filters, aggregateLevel = "teacher") {
    ...
    const supplyTransactions = transactions.filter((t) => {
      const form = String(t.form || "").toLowerCase();
      return (
        form.includes("amazon") ||
        form.includes("warehouse") ||
        form.includes("supply") ||
        form.includes("supplies")
      );
    });
    ...
    return {
      ...
      totalAmount: supplyTransactions.reduce(
        (sum, t) => sum + (t.amount || 0),
        0,
      ),
      byTeacher:
        aggregateLevel === "teacher"
          ? Object.values(byTeacher).sort(
              (a, b) => b.totalAmount - a.totalAmount,
            )
          : null,
      byDepartment:
        aggregateLevel === "department"
          ? Object.values(byDepartment).sort(
              (a, b) => b.totalAmount - a.totalAmount,
            )
          : null,
      byDivision:
        aggregateLevel === "division"
          ? Object.values(byDivision).sort(
              (a, b) => b.totalAmount - a.totalAmount,
            )
          : null,
  ...
  getTransactionDataForReports(filters) {
    ...
    try {
      ...
      return data
        .map((row) => {
          const form = String(row[cols.Form] || "");
          return {
            transactionId: row[cols.TransactionID],
            date: row[cols.ProcessedOn], // Map directly with what we know
            division: "ALL", // placeholder
            department: row[cols.Organization] || "General",
            ...
            status: "Approved",
            requestor: row[cols.Requestor] || row[cols.Email] || "Unknown",
          };
        })
        .filter((t) => {
          // Apply filters
  ...
  getMockReportTransactions() {
    return [
      {
        transactionId: "CUR-001",
        date: new Date(),
        division: "US",
        department: "Math",
        organization: "Curriculum",
        vendor: "Pearson",
        description: "Algebra II Textbooks",
        form: "Curriculum",
        amount: 2500,
        status: "Approved",
        requestor: "jsmith@keswickchristian.org",
      },
      {
        transactionId: "CUR-002",
        date: new Date(),
        division: "US",
        department: "Math",
        organization: "Curriculum",
        vendor: "McGraw Hill",
        description: "Chemistry Lab Supplies",
        form: "Curriculum",
        amount: 1800,
        status: "Approved",
        requestor: "mjones@keswickchristian.org",
      },
      {
        ...
        amount: 1200,
        status: "Approved",
        requestor: "kbrown@keswickchristian.org",
      },
      {
        ...
        amount: 150,
        status: "Approved",
        requestor: "lwhite@keswickchristian.org",
      },
      {
        transactionId: "WH-001",
        date: new Date(),
        division: "LS",
        department: "Grade 4",
        organization: "Classroom",
        vendor: "County Warehouse",
        description: "Paper and Markers",
        form: "Warehouse",
        amount: 85,
        status: "Approved",
        requestor: "dgreen@keswickchristian.org",
      },
      {
        transactionId: "ADM-001",
        date: new Date(),
        division: "AD",
        ...
        amount: 320,
        status: "Approved",
        requestor: "sneel@keswickchristian.org",
      },
    ];
  }
    ...
    return {
      ...
      alerts: [
        ...
        },
        {
          ...
          category: "processing",
          message:
            "Warehouse order processing delayed — expected 48hr turnaround",
          timestamp: new Date(),
        },
      ],
      trends: {
  ...
  getMockTransactions(count) {
    ...
    const allTransactions = [
      {
        transactionId: "AMZ-US-0115",
        date: new Date("2026-01-15"),
        division: "US",
        department: "Math",
        vendor: "Amazon",
        description: "Amazon - 30 TI-84 Plus CE Calculators",
        amount: 3450,
        tac: 3450,
        status: "Approved",
        submitter: "nstratis@keswickchristian.org",
      },
      {
        transactionId: "CUR-US-0118",
        date: new Date("2026-01-18"),
        division: "US",
        department: "Math",
        organization: "Curriculum",
        vendor: "Pearson",
        description: "Curriculum - Pearson Algebra II Textbooks",
        amount: 4200,
        tac: 0,
        status: "Approved",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0122",
        date: new Date("2026-01-22"),
        division: "US",
        department: "Science",
        vendor: "Amazon",
        description: "Amazon - Chemistry Lab Goggles (class set of 30)",
        amount: 285,
        tac: 285,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      },
      {
        transactionId: "WHS-LS-0220",
        ...
        tac: 1200,
        status: "Approved",
        submitter: "kbrown@keswickchristian.org",
      },
      {
        transactionId: "AMZ-US-0205",
        date: new Date("2026-02-05"),
        division: "US",
        department: "Science",
        vendor: "Amazon",
        description: "Amazon - Dissection Kit Refills x15",
        amount: 675,
        tac: 675,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "FLD-LS-0305",
        date: new Date("2026-03-05"),
        division: "LS",
        department: "Grade 5",
        vendor: "Kennedy Space Center",
        description: "Field Trip - Kennedy Space Center (Grade 5)",
        amount: 2800,
        tac: 2800,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      },
      {
        transactionId: "ADM-AD-0210",
        date: new Date("2026-02-10"),
        division: "AD",
        department: "Business Office",
        vendor: "FACTS Management",
        description: "Admin - Annual FACTS License Renewal",
        amount: 8500,
        tac: 0,
        status: "Approved",
        submitter: "mtrotter@keswickchristian.org",
      },
      {
        transactionId: "AMZ-US-0228",
        date: new Date("2026-02-28"),
        division: "US",
        department: "English",
        vendor: "Amazon",
        description: 'Amazon - Novel Sets: "To Kill a Mockingbird" x35',
        amount: 315,
        tac: 0,
        status: "Approved",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "CUR-US-0302",
        date: new Date("2026-03-02"),
        division: "US",
        department: "History",
        vendor: "National Geographic",
        description: "Curriculum - AP US History Document Reader",
        amount: 1890,
        tac: 0,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0303",
        date: new Date("2026-03-03"),
        division: "US",
        department: "Art",
        vendor: "Amazon",
        description: "Amazon - Acrylic Paint Set (48 colors) x10",
        amount: 420,
        tac: 420,
        status: "Pending",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "WHS-LS-0304",
        date: new Date("2026-03-04"),
        division: "LS",
        department: "Grade 1",
        vendor: "County Warehouse",
        description: "Warehouse - Glue Sticks Bulk Pack x200",
        amount: 89,
        tac: 0,
        status: "Processing",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-KK-0225",
        date: new Date("2026-02-25"),
        division: "KK",
        department: "PreK4",
        vendor: "Amazon",
        description: "Amazon - Play-Doh Classroom Pack (24 tubs)",
        amount: 148,
        tac: 148,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "FLD-KK-0310",
        date: new Date("2026-03-10"),
        division: "KK",
        department: "Kindergarten",
        vendor: "Tampa Zoo",
        description: "Field Trip - Tampa Zoo (Kindergarten)",
        amount: 450,
        tac: 450,
        status: "Pending",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0306",
        date: new Date("2026-03-06"),
        division: "US",
        department: "Music",
        vendor: "Amazon",
        description: "Amazon - Sheet Music: Spring Concert Collection",
        amount: 225,
        tac: 225,
        status: "Pending",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "ADM-AD-0301",
        date: new Date("2026-03-01"),
        division: "AD",
        department: "IT",
        vendor: "CDW",
        description: "Admin - Replacement Chromebook Chargers x25",
        amount: 625,
        tac: 0,
        status: "Approved",
        submitter: "mtrotter@keswickchristian.org",
      ...
      {
        transactionId: "WHS-US-0307",
        date: new Date("2026-03-07"),
        division: "US",
        department: "PE",
        vendor: "County Warehouse",
        description: "Warehouse - Dodgeballs and Cones Set",
        amount: 210,
        tac: 210,
        status: "Processing",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-LS-0308",
        date: new Date("2026-03-08"),
        division: "LS",
        department: "Specials",
        vendor: "Amazon",
        description: "Amazon - STEM Building Blocks Classroom Kit",
        amount: 389,
        tac: 389,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "CUR-US-0309",
        date: new Date("2026-03-09"),
        division: "US",
        department: "Bible",
        vendor: "David C Cook",
        description: "Curriculum - Bible Study Workbooks (Grade 10)",
        amount: 560,
        tac: 0,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0311",
        date: new Date("2026-03-11"),
        division: "US",
        department: "World Languages",
        vendor: "Amazon",
        description: "Amazon - Spanish-English Dictionaries x20",
        amount: 340,
        tac: 0,
        status: "Pending",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "ADM-AD-0312",
        date: new Date("2026-03-12"),
        division: "AD",
        department: "Facilities",
        vendor: "Home Depot",
        description: "Admin - Maintenance Supplies (HVAC Filters)",
        amount: 475,
        tac: 0,
        status: "Processing",
        submitter: "mtrotter@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0125",
        date: new Date("2026-01-25"),
        division: "US",
        department: "Media",
        vendor: "Amazon",
        description: "Amazon - Podcast Microphones x4",
        amount: 520,
        tac: 520,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "WHS-KK-0215",
        date: new Date("2026-02-15"),
        division: "KK",
        department: "Toddlers",
        vendor: "County Warehouse",
        description: "Warehouse - Finger Paint and Easel Paper",
        amount: 95,
        tac: 0,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-KK-0130",
        date: new Date("2026-01-30"),
        division: "KK",
        department: "Infants",
        vendor: "Amazon",
        description: "Amazon - Sensory Play Mat (set of 6)",
        amount: 234,
        tac: 234,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "FLD-US-0218",
        date: new Date("2026-02-18"),
        division: "US",
        department: "History",
        vendor: "Ybor City Museum",
        description: "Field Trip - Ybor City Museum (AP History)",
        amount: 680,
        tac: 680,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "CUR-LS-0203",
        date: new Date("2026-02-03"),
        division: "LS",
        department: "Grade 2",
        vendor: "Scholastic",
        description: "Curriculum - Guided Reading Level J-M Book Set",
        amount: 1250,
        tac: 0,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "ADM-AD-0115",
        date: new Date("2026-01-15"),
        division: "AD",
        department: "Development",
        vendor: "Constant Contact",
        description: "Admin - Annual Email Marketing Platform License",
        amount: 1800,
        tac: 0,
        status: "Approved",
        submitter: "mtrotter@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-LS-0208",
        date: new Date("2026-02-08"),
        division: "LS",
        department: "Grade 4",
        vendor: "Amazon",
        description: "Amazon - Fraction Manipulatives Kit x6",
        amount: 198,
        tac: 198,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "WHS-US-0212",
        date: new Date("2026-02-12"),
        division: "US",
        department: "Science",
        vendor: "County Warehouse",
        description: "Warehouse - Lab Notebooks x120",
        amount: 360,
        tac: 0,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-KK-0305",
        date: new Date("2026-03-05"),
        division: "KK",
        department: "PreK3",
        vendor: "Amazon",
        description: "Amazon - Wooden Block Set (200 piece)",
        amount: 165,
        tac: 165,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "CUR-US-0128",
        date: new Date("2026-01-28"),
        division: "US",
        department: "Science",
        vendor: "Carolina Biological",
        description: "Curriculum - AP Biology Lab Kit (Semester 2)",
        amount: 2150,
        tac: 0,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0201",
        date: new Date("2026-02-01"),
        division: "US",
        department: "Math",
        vendor: "Amazon",
        description: "Amazon - Graphing Calculator Screen Protectors x30",
        amount: 90,
        tac: 90,
        status: "Approved",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "WHS-LS-0110",
        date: new Date("2026-01-10"),
        division: "LS",
        department: "Grade 1",
        vendor: "County Warehouse",
        description: "Warehouse - Construction Paper Bulk (20 colors)",
        amount: 124,
        tac: 0,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-AD-0120",
        date: new Date("2026-01-20"),
        division: "AD",
        department: "IT",
        vendor: "Amazon",
        description: "Amazon - USB-C Hubs for Teacher Laptops x10",
        amount: 350,
        tac: 0,
        status: "Approved",
        submitter: "mtrotter@keswickchristian.org",
      ...
      {
        transactionId: "FLD-LS-0215",
        date: new Date("2026-02-15"),
        division: "LS",
        department: "Grade 3",
        vendor: "MOSI",
        description: "Field Trip - Museum of Science & Industry (Grade 3)",
        amount: 1650,
        tac: 1650,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0310",
        date: new Date("2026-03-10"),
        division: "US",
        department: "English",
        vendor: "Amazon",
        description: "Amazon - Composition Notebooks x100",
        amount: 275,
        tac: 0,
        status: "Approved",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "CUR-KK-0201",
        date: new Date("2026-02-01"),
        division: "KK",
        department: "Kindergarten",
        vendor: "Scholastic",
        description: "Curriculum - Sight Words Flash Card System",
        amount: 320,
        tac: 0,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "WHS-US-0120",
        date: new Date("2026-01-20"),
        division: "US",
        department: "Art",
        vendor: "County Warehouse",
        description: "Warehouse - Drawing Pencil Set (H-9B) x25",
        amount: 175,
        tac: 0,
        status: "Approved",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-LS-0125",
        date: new Date("2026-01-25"),
        division: "LS",
        department: "Grade 5",
        vendor: "Amazon",
        description: "Amazon - Science Fair Display Boards x30",
        amount: 210,
        tac: 210,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "ADM-AD-0205",
        date: new Date("2026-02-05"),
        division: "AD",
        department: "Business Office",
        vendor: "Staples",
        description: "Admin - Office Supplies Quarterly Order",
        amount: 425,
        tac: 0,
        status: "Approved",
        submitter: "mtrotter@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0215",
        date: new Date("2026-02-15"),
        division: "US",
        department: "PE",
        vendor: "Amazon",
        description: "Amazon - Resistance Bands Set (class pack)",
        amount: 189,
        tac: 189,
        status: "Approved",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "CUR-US-0220",
        date: new Date("2026-02-20"),
        division: "US",
        department: "World Languages",
        vendor: "Vista Higher Learning",
        description: "Curriculum - AP Spanish Workbooks x25",
        amount: 1125,
        tac: 0,
        status: "Approved",
        submitter: "nstratis@keswickchristian.org",
      ...
      {
        transactionId: "WHS-KK-0310",
        date: new Date("2026-03-10"),
        division: "KK",
        department: "PreK4",
        vendor: "County Warehouse",
        description: "Warehouse - Sanitizing Wipes Bulk Case",
        amount: 68,
        tac: 0,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0105",
        date: new Date("2026-01-05"),
        division: "US",
        department: "Music",
        vendor: "Amazon",
        description: "Amazon - Ukulele Strings Replacement Set x20",
        amount: 140,
        tac: 140,
        status: "Approved",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "FLD-US-0312",
        date: new Date("2026-03-12"),
        division: "US",
        department: "Science",
        vendor: "Florida Aquarium",
        description: "Field Trip - Florida Aquarium (Marine Bio)",
        amount: 1200,
        tac: 1200,
        status: "Pending",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-LS-0301",
        date: new Date("2026-03-01"),
        division: "LS",
        department: "Grade 2",
        vendor: "Amazon",
        description: "Amazon - Math Counting Bears Set x8",
        amount: 176,
        tac: 176,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "ADM-AD-0308",
        date: new Date("2026-03-08"),
        division: "AD",
        department: "Facilities",
        vendor: "Grainger",
        description: "Admin - Replacement Light Fixtures (LED) x12",
        amount: 960,
        tac: 0,
        status: "Approved",
        submitter: "mtrotter@keswickchristian.org",
      ...
      {
        transactionId: "CUR-LS-0310",
        date: new Date("2026-03-10"),
        division: "LS",
        department: "Grade 4",
        vendor: "Houghton Mifflin",
        description: "Curriculum - Social Studies Atlas Set x30",
        amount: 840,
        tac: 0,
        status: "Approved",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-US-0313",
        date: new Date("2026-03-13"),
        division: "US",
        department: "Bible",
        vendor: "Amazon",
        description: "Amazon - Devotional Journals x35",
        amount: 385,
        tac: 0,
        status: "Processing",
        submitter: "bendrulat@keswickchristian.org",
      ...
      {
        transactionId: "WHS-LS-0313",
        date: new Date("2026-03-13"),
        division: "LS",
        department: "Specials",
        vendor: "County Warehouse",
        description: "Warehouse - Laminating Film Rolls x6",
        amount: 132,
        tac: 0,
        status: "Processing",
        submitter: "sneel@keswickchristian.org",
      ...
      {
        transactionId: "AMZ-KK-0313",
        date: new Date("2026-03-13"),
        division: "KK",
        department: "Toddlers",
        vendor: "Amazon",
        description: "Amazon - Soft Stacking Rings (set of 12)",
        amount: 108,
        tac: 108,
        status: "Pending",
        submitter: "sneel@keswickchristian.org",
  ...
  getMockCategoricalSpending() {
    ...
    return [
      ...
      {
        category: "Field Trips & Activities",
        amount: 261000,
        percentage: 10.0,
      },
      {
  ...
  getMockDepartmentSummary(divisions) {
    ...
    (divisions || ["US"]).forEach(function (div) {
      if (deptData[div]) {
        deptData[div].forEach(function (dept) {
          result.push(dept);
        });
      }
    });
  ...
  getMockTACAnalysis(divisions) {
    ...
    (divisions || ["US"]).forEach(function (div) {
      if (tacData[div]) {
        ...
        tacData[div].byGrade.forEach(function (g) {
          combined.byGrade.push(g);
        });
      }
    });
    return combined;
  ...
  getMockApprovalQueue(divisions) {
    const allApprovals = [
      {
        id: "APR-2026-1001",
        date: new Date("2026-03-08"),
        requester: "bendrulat@keswickchristian.org",
        division: "US",
        department: "Science",
        amount: 1200,
        description: "Field Trip - Florida Aquarium (Marine Bio)",
      ...
      },
      {
        id: "APR-2026-1002",
        date: new Date("2026-03-09"),
        requester: "nstratis@keswickchristian.org",
        division: "US",
        department: "Art",
        amount: 420,
        description: "Amazon - Acrylic Paint Set (48 colors) x10",
      ...
      },
      {
        id: "APR-2026-1003",
        date: new Date("2026-03-10"),
        requester: "sneel@keswickchristian.org",
        division: "KK",
        department: "Kindergarten",
        amount: 450,
        description: "Field Trip - Tampa Zoo (Kindergarten)",
      ...
      },
      {
        id: "APR-2026-1007",
        date: new Date("2026-03-12"),
        requester: "mtrotter@keswickchristian.org",
        division: "AD",
        department: "Facilities",
        amount: 475,
        description: "Admin - Maintenance Supplies (HVAC Filters)",
    ...
    return allApprovals.filter(function (a) {
      return divisions.indexOf(a.division) !== -1;
    });
  }
  ...
  getMockDivisionAlerts(divisions) {
    const allAlerts = [
      {
        type: "warning",
        message:
          "Science department at 85% budget utilization — review remaining allocations",
        division: "US",
        department: "Science",
    ...
    return allAlerts.filter(function (a) {
      return !a.division || divisions.indexOf(a.division) !== -1;
    });
  }
  ...
  getMockTACBreakdown(departments) {
    ...
    var tacByDept = {
      Science: {
        technology: { a: 18000, s: 10800 },
        activities: { a: 8000, s: 4800 },
        consumables: { a: 6500, s: 3900 },
      },
      Math: {
        technology: { a: 15000, s: 9000 },
        activities: { a: 5000, s: 3000 },
        consumables: { a: 5000, s: 3000 },
      },
      English: {
        technology: { a: 8000, s: 4000 },
        activities: { a: 4000, s: 2000 },
        consumables: { a: 6000, s: 3600 },
      },
      History: {
        technology: { a: 7000, s: 3500 },
        activities: { a: 6000, s: 3600 },
        consumables: { a: 4000, s: 2400 },
      },
      Art: {
        technology: { a: 5000, s: 1750 },
        activities: { a: 3000, s: 1050 },
        consumables: { a: 8000, s: 2800 },
      },
      Music: {
        technology: { a: 9000, s: 4590 },
        activities: { a: 5000, s: 2550 },
        consumables: { a: 3000, s: 1530 },
      },
      PE: {
        technology: { a: 3000, s: 1410 },
        activities: { a: 7000, s: 3290 },
        consumables: { a: 4000, s: 1880 },
      },
      Media: {
        technology: { a: 16000, s: 10720 },
        activities: { a: 3000, s: 2010 },
        consumables: { a: 2000, s: 1340 },
      },
      Bible: {
        technology: { a: 6000, s: 3720 },
        activities: { a: 4000, s: 2480 },
        consumables: { a: 3000, s: 1860 },
      },
      "World Languages": {
        technology: { a: 8000, s: 3520 },
        activities: { a: 3000, s: 1320 },
        consumables: { a: 3000, s: 1320 },
      },
    };
    // Default fallback for departments not listed
    var defaultTac = {
      technology: { a: 5000, s: 2500 },
      activities: { a: 3000, s: 1500 },
      consumables: { a: 2000, s: 1000 },
    };
    ...
    breakdown.technology.available =
      breakdown.technology.allocated - breakdown.technology.spent;
    breakdown.activities.available =
      breakdown.activities.allocated - breakdown.activities.spent;
    breakdown.consumables.available =
      breakdown.consumables.allocated - breakdown.consumables.spent;
