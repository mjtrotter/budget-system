"""Scenario B1: Curriculum Request ($150).

Tests that a Curriculum request correctly routes to the 
...
from helpers.forms import open_form, fill_curriculum_form, submit_form


def inject(context) -> None:
    """Set up for B1: Teacher with $2000 budget."""
...
def execute(context) -> None:
    """Submit Curriculum form for $150."""
    page = open_form(context, CURRICULUM_FORM)
    fill_curriculum_form(page, resource_name="Test Resource B1", grade_levels="6-8", quantity=10, unit_price="15.00")
    submit_form(page)
...
def verify(context) -> str:
    ...
    owa.screenshot(path=os.path.join(EVIDENCE_DIR, "b1_email.png"))
    
    if click_approval_link(owa):
        owa.wait_for_timeout(5000)
        owa.screenshot(path=os.path.join(EVIDENCE_DIR, "b1_approval_landed.png"))
        
        body_landed = owa.locator("body").inner_text().lower()
        if "approved" not in body_landed:
            result = "FAIL"
            details.append("Curriculum approval landing page failed")
    else:
        result = "FAIL"
        details.append("Curriculum approval link not found in email")

    owa.close()
    return f"B1: {result}" + (f" — {'; '.join(details)}" if details else "")
