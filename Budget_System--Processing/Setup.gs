...
function updateWebAppUrl() {
  const props = PropertiesService.getScriptProperties();
  const newUrl = 'https://script.google.com/a/keswickchristian.org/macros/s/AKfycbwL5PpHn1P8GtppiZOnopXwh_E89Vma9LKi2aarTPlvlYdv8FbuU1M5N3-yt67yCzgadg/exec';
...
function createFreshHubs() {
  ...
  ledger.appendRow([
    'TransactionID', 'OrderID', 'ProcessedOn', 'Requestor', 'Approver',
  ...
  userDir.appendRow([
    'Email', 'FirstName', 'LastName', 'Role', 'Department', 'Division',
    'Approver', 'BudgetAllocated', 'BudgetSpent', 'BudgetEncumbered',
    'BudgetRemaining', 'UtilizationRate', 'Active', 'LastModified'
  ]);
  ...
  try {
    ...
    if (oldUserDir && oldUserDir.getLastRow() > 1) {
      const userData = oldUserDir.getRange(2, 1, oldUserDir.getLastRow() - 1, oldUserDir.getLastColumn()).getValues();
      if (userData.length > 0) {
        userDir.getRange(2, 1, userData.length, userData[0].length).setValues(userData);
    ...
    console.warn(`   ⚠️ UserDirectory created but migration failed: ${e.message}`);
  ...
  orgBudgets.appendRow([
    'Organization', 'BudgetAllocated', 'BudgetSpent', 'BudgetEncumbered',
    'BudgetAvailable', 'Approver', 'Active', 'LastModified'
  ]);
  ...
  try {
    ...
    if (oldOrgBudgets && oldOrgBudgets.getLastRow() > 1) {
      const orgData = oldOrgBudgets.getRange(2, 1, oldOrgBudgets.getLastRow() - 1, oldOrgBudgets.getLastColumn()).getValues();
      if (orgData.length > 0) {
        orgBudgets.getRange(2, 1, orgData.length, orgData[0].length).setValues(orgData);
        console.log(`   ✅ OrganizationBudgets (migrated ${orgData.length} orgs)`);
      }
    } else {
      console.log('   ✅ OrganizationBudgets (empty)');
    }
  } catch (e) {
    console.warn(`   ⚠️ OrganizationBudgets created but migration failed: ${e.message}`);
  }

  // SystemLog
  const sysLog = budgetHub.insertSheet('SystemLog');
  sysLog.appendRow(['Timestamp', 'Action', 'User', 'Amount', 'Details', 'Before', 'After', 'Status']);
  ...
  amazon.appendRow([
    ...
    'Item 1 - Description', 'Item 1 - Amazon URL', 'Item 1 - Quantity', 'Item 1 - Unit Price ($)',
    'Add another item?',
    'Item 2 - Description', 'Item 2 - Amazon URL', 'Item 2 - Quantity', 'Item 2 - Unit Price ($)',
    'Add another item?',
    'Item 3 - Description', 'Item 3 - Amazon URL', 'Item 3 - Quantity', 'Item 3 - Unit Price ($)',
    'Add another item?',
    'Item 4 - Description', 'Item 4 - Amazon URL', 'Item 4 - Quantity', 'Item 4 - Unit Price ($)',
    'Add another item?',
    'Item 5 - Description', 'Item 5 - Amazon URL', 'Item 5 - Quantity', 'Item 5 - Unit Price ($)'
  ]);
  ...
  warehouse.appendRow([
    'Timestamp', 'Email Address',
    'Item 1 - Catalog ID', 'Item 1 - Quantity',
    'Add another item?',
    'Item 2 - Catalog ID', 'Item 2 - Quantity',
    'Add another item?',
    'Item 3 - Catalog ID', 'Item 3 - Quantity',
    'Add another item?',
    'Item 4 - Catalog ID', 'Item 4 - Quantity',
    'Add another item?',
    'Item 5 - Catalog ID', 'Item 5 - Quantity'
  ]);
  ...
  catalog.appendRow(['Stock Number', 'Item Description', 'Price', 'UOM', 'Category']);
  ...
  try {
    ...
    if (oldCatalog && oldCatalog.getLastRow() > 1) {
      const catData = oldCatalog.getRange(2, 1, oldCatalog.getLastRow() - 1, oldCatalog.getLastColumn()).getValues();
      if (catData.length > 0) {
        catalog.getRange(2, 1, catData.length, catData[0].length).setValues(catData);
        console.log(`   ✅ WarehouseCatalog (migrated ${catData.length} items)`);
      }
    } else {
      console.log('   ✅ WarehouseCatalog (empty)');
    }
  } catch (e) {
    console.warn(`   ⚠️ WarehouseCatalog created but migration failed: ${e.message}`);
  }

  // AutomatedQueue
  const autoQueue = autoHub.insertSheet('AutomatedQueue');
  autoQueue.appendRow([
    'TransactionID', 'Requestor', 'RequestType', 'Department', 'Division',
    'Amount', 'Description', 'Status', 'Requested', 'Approved', 'Processed', 'ResponseID'
  ]);
  ...
  fieldTrip.appendRow([
    'Timestamp', 'Email Address',
    'Field Trip Destination', 'Trip Date', 'Number of Students',
    'Transportation', 'Quoted Cost', 'Upload Quote/PO'
  ]);
  ...
  curriculum.appendRow([
    'Timestamp', 'Email Address',
    'Type of Curriculum Request', 'Resource Name/Title', 'Grade Level(s)',
    'Quantity', 'Unit Price ($)', 'Entry Method',
    'Publisher/Vendor', 'Link to resource', 'Purpose',
    'Upload PDF Quote', 'Purpose (PDF path)'
  ]);
  ...
  admin.appendRow([
    'Timestamp', 'Email Address',
    'Purchase Description', 'Amount ($)', 'Category',
    'Upload the PO/Receipt', 'Additional Notes'
  ]);
  ...
  [budgetHub, autoHub, manualHub].forEach(hub => {
    uatUsers.forEach(user => {
      try {
        ...
        console.warn(`   ⚠️ Could not add ${user} to ${hub.getName()}: ${e.message}`);
      }
    });
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log('');
  console.log('UPDATE THESE IDs IN Config.gs, Invoicing Arm/Main.gs, Dashboard Arm/Dashboard_BE.gs:');
  console.log('');
  ...
  console.log(`  Budget Hub:    https://docs.google.com/spreadsheets/d/${budgetHubId}/edit`);
  console.log(`  Automated Hub: https://docs.google.com/spreadsheets/d/${autoHubId}/edit`);
  console.log(`  Manual Hub:    https://docs.google.com/spreadsheets/d/${manualHubId}/edit`);
  console.log('');
  ...
  console.log('');
  console.log('✅ New IDs also saved to Script Properties (CONFIG will pick them up via getDyn)');
}
...
function cleanupOldHubs() {
  ...
  const oldHubs = [
    { name: 'Old Budget Hub', id: '1wbv44RU18vJXlImWwxf4VRX932LgWCTBEn6JNws9HhQ' },
    { name: 'Old Automated Hub', id: '1CfktVXDNTY499U7zgkBVJ0DyBePH_BsYJeMtwyaxolM' },
    { name: 'Old Manual Hub', id: '1-t7YnyVvk0vxqbKGNteNHOQ92XOJNme4eJPjxVRVI5M' }
  ];
  ...
  for (const hub of newHubs) {
    try {
      ...
      console.log(`   ✅ ${hub.name}: "${ss.getName()}" (${ss.getSheets().length} sheets)`);
    } catch (e) {
      console.error(`   ❌ ${hub.name} (${hub.id}) NOT ACCESSIBLE: ${e.message}`);
  ...
  console.log('');
  console.log('✅ Cleanup complete. Old hubs are in Trash (recoverable for 30 days).');
